<script lang="ts">
  ...
  $: englishLanguagePath = currentLanguage === 'ar' 
    ? currentPagePath.replace(/^\/ar/, '/en') 
    : currentPagePath;
  $: arabicLanguagePath = currentLanguage === 'en'
...
<header>
  <div class="fixed top-0 bg-white-pure w-full pt-4 pb-3 z-50">
    <div class="grid md:grid-cols-3 items-center h-full px-4 w-full max-w-screen-xl mx-auto text-black">
      ...
      <div class="language-switcher text-center font-martina">
        <a href={englishLanguagePath} on:click={(e) => switchLanguage(e, englishLanguagePath)}>English</a> | 
        <a href={arabicLanguagePath} on:click={(e) => switchLanguage(e, arabicLanguagePath)}>العربية</a>
      </div>
  ...
  <nav class="w-full flex px-4 pt-4 justify-between items-center h-12 md:h-24">
    ...
    <ul role="navigation" aria-label="Main" class="fixed w-full items-center h-screen inset-0 z-30 bg-white-pure justify-center flex-row gap-6  {open ? 'flex flex-col' : 'hidden' }" >
